...
public class Mates
{
	...
	public static float distancia_segmento_punto(Punto r1, Punto r2, Punto p3)
	{
		...
		// c puede tomar el valor de los extremos del segmento o i (si i esta contenido en el segmento)
		float xc, yc;
		if (p1.x() == p2.x())
			if (p1.y() < p2.y())
				if (y >= p2.y()) 		{ xc = p2.x(); yc = p2.y(); }
				else if (y <= p1.y()) 	{ xc = p1.x(); yc = p1.y(); }
				else 					{ xc = x; 	 yc = y;		}
			else 
				if (y >= p1.y()) 		{ xc = p1.x(); yc = p1.y(); }
				else if (y <= p2.y()) 	{ xc = p2.x(); yc = p2.y(); }
				else 					{ xc = x; 	 yc = y; 		}
		else if (x <= x1) 				{ xc = p1.x(); yc = p1.y(); }
		else if (x >= x2) 				{ xc = p2.x(); yc = p2.y(); }
		else 							{ xc = x; 	 yc = y; 		}
