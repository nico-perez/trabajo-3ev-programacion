...
public class CapaRaster extends Capa {
    ...
    public void combinar_pixeles(Color[][] fondo) {
        ...
        if (p_ini.x() >= fondo.length)
            p_ini.setX(fondo.length);
        if (p_ini.y() >= fondo[0].length)
            p_ini.setY(fondo[0].length);
        if (p_fin.x() >= fondo.length)
            p_fin.setX(fondo.length);
        if (p_fin.y() >= fondo[0].length)
            p_fin.setY(fondo[0].length);
